...
const ComedyClassics = (props) => {
  const API_KEY = import.meta.env.VITE_YOUTUBE_API_KEY;
  const PLAYLIST_ID = "PL-lPJw3tb8iDZy_3jRn7lx1nvZZSCvuc7";
  ...
  const fetchTumbnails = async (Pagetoken = "") => {
    ...
    try {
      ...
      const tumbnailsWithStats = playResponse.data.items.map((item, index) => ({
        ...item.snippet,
  ...
  return (
    <div style={{ marginTop: "0px" }} className="TopratedShows">
      <div className="headlinesDiv">
        <p className="headline">Comedy Classics</p>
        <p className="miniheadline">
          Catch the crowd-favorites in stand-up comedy—only the best make this
          list!
        </p>
      </div>
      <div
      ...
      >
        {Tumbnails.map((Tumbnail, index) => (
          <div className="topRatedVideo" key={Tumbnail.resourceId.videoId + "comedy"}>
            <img
            ...
            <div className="videoDetails">
              <div className="videoBtns">
                <NavLink to="/StandUp" state={{ videoIndex: index }}>
