...
const TopRatedShows = (props) => {
  ...
  const PLAYLIST_ID = "PL-lPJw3tb8iAZ1v8-tVi3RQYYmwWA8SLI";
  ...
  const fetchTumbnails = async (Pagetoken = "") => {
    ...
    try {
      const playResponse = await axios.get(
        `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${PLAYLIST_ID}&key=${API_KEY}&maxResults=6&pageToken=${Pagetoken}`
      );
  ...
  return (
    <div className="TopratedShows">
      <div className="headlinesDiv">
        <p className="headline">Top-Rated Shows</p>
        <p className="miniheadline">
          Check out the top-rated podcasts, chosen for their impact and
          popularity!
        </p>
      </div>
      <div
      ...
      >
        {Tumbnails.map((Tumbnail, index) => (
          <div className="topRatedVideo" key={Tumbnail.resourceId.videoId + "topshows"}>
            <img
            ...
            <div className="videoDetails">
              <div className="videoBtns">
                <NavLink to="/Podcasts" state={{ videoIndex: index }}>
